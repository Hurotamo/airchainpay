...
use chacha20poly1305::{ChaCha20Poly1305, aead::Aead as ChaChaAead, Nonce as ChaChaNonce};
use rand::RngCore;
use zeroize::Zeroize;
use std::sync::Arc;
...
impl EncryptionManager {
    ...
    pub fn new() -> Result<Self, WalletError> {
        ...
        let aes_gcm = Aes256Gcm::new_from_slice(&aes_key)
            .map_err(|e| WalletError::crypto(format!("Failed to initialize AES-GCM: {}", e)))?;
        
        let chacha20 = ChaCha20Poly1305::new_from_slice(&chacha_key)
    ...
    pub async fn encrypt_wallet_data(&self, wallet_data: &[u8], password: &str) -> Result<Vec<u8>, WalletError> {
        ...
        let temp_aes = Aes256Gcm::new_from_slice(&key)
