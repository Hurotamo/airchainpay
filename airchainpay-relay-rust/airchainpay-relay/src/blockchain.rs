...
use ethers::{
    ...
    types::{Address, U256, H256, BlockNumber, Log, TransactionReceipt as EthersReceipt, TxHash},
    prelude::*,
    ...
    pub fn new(config: Config) -> Result<Self> {
        ...
        for (chain_id, chain_config) in &config.supported_chains {
            ...
            if !chain_config.contract_address.is_empty() {
                ...
                let contract = Contract::new(contract_address, include_bytes!("../abi/AirChainPay.json"), Arc::new(provider));
    ...
    pub async fn send_transaction(&self, tx_bytes: Vec<u8>, rpc_url: &str, chain_id: u64) -> Result<String> {
        ...
        if let Some(handler) = &self.critical_error_handler {
            return handler.execute_critical_operation(
                ...
                context,
            ).await.map_err(|e| anyhow!("Critical error in blockchain transaction: {}", e.error_message));
        }
    ...
    pub async fn get_transaction_receipt(&self, tx_hash: &str, chain_id: u64) -> Result<TransactionReceipt> {
        ...
        if let Some(handler) = &self.critical_error_handler {
            return handler.execute_critical_operation(
                ...
                context,
            ).await.map_err(|e| anyhow!("Critical error getting transaction receipt: {}", e.error_message));
        }
    ...
    pub async fn estimate_gas(&self, to: &str, data: &str, chain_id: u64) -> Result<GasEstimate> {
        ...
        if let Some(handler) = &self.critical_error_handler {
            return handler.execute_critical_operation(
                ...
                context,
            ).await.map_err(|e| anyhow!("Critical error estimating gas: {}", e.error_message));
        }
    ...
    pub async fn get_balance(&self, address: &str, chain_id: u64) -> Result<U256> {
        ...
        if let Some(handler) = &self.critical_error_handler {
            return handler.execute_critical_operation(
                ...
                context,
            ).await.map_err(|e| anyhow!("Critical error getting balance: {}", e.error_message));
        }
    ...
    pub async fn get_nonce(&self, address: &str, chain_id: u64) -> Result<U256> {
        ...
        if let Some(handler) = &self.critical_error_handler {
            return handler.execute_critical_operation(
                ...
                context,
            ).await.map_err(|e| anyhow!("Critical error getting nonce: {}", e.error_message));
        }
    ...
    pub async fn get_block_number(&self, chain_id: u64) -> Result<U256> {
        ...
        if let Some(handler) = &self.critical_error_handler {
            return handler.execute_critical_operation(
                ...
                context,
            ).await.map_err(|e| anyhow!("Critical error getting block number: {}", e.error_message));
        }
    ...
    pub async fn get_contract_events(&self, contract_address: &str, from_block: u64, to_block: u64, chain_id: u64) -> Result<Vec<Log>> {
        ...
        if let Some(handler) = &self.critical_error_handler {
            return handler.execute_critical_operation(
                ...
                context,
            ).await.map_err(|e| anyhow!("Critical error getting contract events: {}", e.error_message));
        }
    ...
    pub async fn is_transaction_confirmed(&self, tx_hash: &str, chain_id: u64, confirmations: u64) -> Result<bool> {
        ...
        if let Some(handler) = &self.critical_error_handler {
            return handler.execute_critical_operation(
                ...
                context,
            ).await.map_err(|e| anyhow!("Critical error checking transaction confirmation: {}", e.error_message));
        }
            ...
            return handler.execute_critical_operation(
                ...
                || async {
                    ...
                    match provider.get_block_number().await {
                        ...
                        Err(e) => {
                            Logger::warn(&format!("Failed to get block number for chain {}: {}", chain_id, e));
                        }
                    }
                    ...
                    match provider.get_gas_price().await {
                        ...
                        Err(e) => {
                            Logger::warn(&format!("Failed to get gas price for chain {}: {}", chain_id, e));
                        }
                    }
                ...
                context,
            ).await.map_err(|e| anyhow!("Critical error getting network info: {}", e.error_message));
        }
        ...
        if let Some(handler) = &self.critical_error_handler {
            return handler.execute_critical_operation(
                ...
                || async {
                    ...
                    match provider.get_block_number().await {
                        Ok(_) => {
                            Logger::info(&format!("Chain {} connection validated successfully", chain_id));
                            Ok(true)
                        }
                        Err(e) => {
                            Logger::error(&format!("Chain {} connection validation failed: {}", chain_id, e));
                            Ok(false)
                        }
                    }
                },
                context,
            ).await.map_err(|e| anyhow!("Critical error validating chain connection: {}", e.error_message));
        }
    ...
    pub async fn wait_for_transaction_receipt(&self, tx_hash: &str, rpc_url: &str, chain_id: u64) -> Result<TransactionReceipt> {
        ...
        if let Some(handler) = &self.critical_error_handler {
            return handler.execute_critical_operation(
                ...
                || async {
                    ...
                    while attempts < max_attempts {
                        match provider.get_transaction_receipt(hash).await {
                            ...
                            Err(e) => {
                                Logger::warn(&format!("Error checking transaction receipt: {}", e));
                                tokio::time::sleep(tokio::time::Duration::from_secs(5)).await;
                                attempts += 1;
                            }
                        }
                ...
                context,
            ).await.map_err(|e| anyhow!("Critical error waiting for transaction receipt: {}", e.error_message));
        }
